...
- `core/settings.py` — única `Settings` (pydantic-settings, `extra=ignore`) + `get_settings()` (lru_cache): `bitrix_client_id`, `bitrix_client_secret`, `app_base_url`. **Config nueva = un campo aquí.**
...
- `integration/bitrix/events.py` — `parse_agent_message(form)`: devuelve `{conversation_id, text, author, files}` solo si `ONIMBOTMESSAGEADD` + `CHAT_ENTITY_TYPE=IA_CONV` + `IS_BOT=N` (ignora el propio bot → evita bucle) + no `SYSTEM=Y`; si no, `None`. `forward_to_platform(msg)`: TODO POST a la plataforma externa (hoy `print`).
- `integration/bitrix/schemas.py` — pydantic. `NewMessage`: `conversation_id`, `text`, `sender=""`, `system=False`, `channel=""`, `account=""`.
- `integration/bitrix/router.py` — prefix `/bitrix`: `POST /install` (guarda tokens; acepta formato con UI `DOMAIN/AUTH_ID/REFRESH_ID` y evento `ONAPPINSTALL` `auth[domain]/auth[access_token]/…`), `POST /bot` (vuelta agente→plataforma), `POST /mensajes` (ida).
- `index.html` — página de pruebas manual.
- `docker-compose.yml` — túnel `ngrok` (URL fija `PUBLIC_URL` del `.env`) → `host:8000`.
